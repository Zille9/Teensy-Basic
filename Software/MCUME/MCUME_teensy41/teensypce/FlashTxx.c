...
int firmware_buffer_init( uint32_t *buffer_addr, uint32_t *buffer_size )
{
  ...
  // KORREKTUR: Wir nutzen den echten 4 MB PSRAM (EXTMEM) als statischen Puffer.
  // Das schont den internen RAM und verhindert, dass das malloc() fehlschlägt!
  static EXTMEM uint8_t multiboot_psram_buffer[4 * 1024 * 1024];
  
  *buffer_addr = (uint32_t)multiboot_psram_buffer;
  *buffer_size = sizeof(multiboot_psram_buffer);
  
  // Den Puffer komplett mit 0xFF vorbefüllen (wie gelöschter Flash)
  memset( (void*)*buffer_addr, 0xFF, *buffer_size ); 
  
  // Gibt RAM_BUFFER_TYPE (2) zurück, damit FlasherX linear ohne Adress-Verschiebung schreibt
  return( RAM_BUFFER_TYPE );
  #endif

  // (Der restliche Flash-Code unten wird für den Teensy 4.1 nun komplett ignoriert)
