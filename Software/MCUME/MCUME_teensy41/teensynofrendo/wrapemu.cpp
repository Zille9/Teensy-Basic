...
void nes_Start(char * filename)
{
	...
	romdata = (char*)emu_SMalloc(romsize);
...
void osd_getinput(void)
{
  ...
  if ( (j & MASK_KEY_USER2) || (hk == 'a') ) // A
    b &= ~0x2000;
  if ( (j & MASK_KEY_USER3) || (hk == 'q') ) // SELECT
    b &= ~0x0001;
  if ( (j & MASK_KEY_USER1) || (hk == 'w') ) // START
    b &= ~0x0008;
  if ( (j & MASK_JOY2_UP) || (j & MASK_JOY1_UP) )
    b &= ~0x0010;
  if ( (j & MASK_JOY2_RIGHT) || (j & MASK_JOY1_RIGHT) )
    b &= ~0x0020;
  if ( (j & MASK_JOY2_DOWN) || (j & MASK_JOY1_DOWN) )
    b &= ~0x0040;
  if ( (j & MASK_JOY2_LEFT) || (j & MASK_JOY1_LEFT) )
    b &= ~0x0080;
  if ( (j & MASK_JOY2_BTN) || (j & MASK_JOY1_BTN) )  // B
    b &= ~0x4000;
