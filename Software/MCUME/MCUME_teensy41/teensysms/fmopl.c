...
static int OPLOpenTable( void )
{
	...
	if( (TL_TABLE = emu_Malloc(TL_MAX*2*sizeof(INT32))) == NULL)
		return 0;
	if( (SIN_TABLE = emu_Malloc(SIN_ENT*4 *sizeof(INT32 *))) == NULL)
	{
		emu_Free(TL_TABLE);
		return 0;
	}
	if( (AMS_TABLE = emu_Malloc(AMS_ENT*2 *sizeof(INT32))) == NULL)
	{
		emu_Free(TL_TABLE);
		emu_Free(SIN_TABLE);
		return 0;
	}
	if( (VIB_TABLE = emu_Malloc(VIB_ENT*2 *sizeof(INT32))) == NULL)
	{
		emu_Free(TL_TABLE);
		emu_Free(SIN_TABLE);
		emu_Free(AMS_TABLE);
